...
@click.option('--aligner', type=click.Choice(['gembs', 'bwameth']), default='gembs', help='Select aligner to be used.')
...
def create_config(use_sample_files, genome_build, aligner, cores_per_job, fastq_dir, reference_fasta, group1, group2, output_dir, target_yaml):
    ...
    cli.config.create_config(use_sample_files, genome_build, aligner, cores_per_job, fastq_dir, reference_fasta, group1, group2, output_dir, target_yaml)
...
def run_snakemake_from_config(dry_run, cores, cluster, nodes, config_yaml):
    ...
    cli.snakemake.run_snakemake_from_config(dry_run, config_yaml, cores, cluster, nodes)
...
@click.argument('group2')
@click.argument('output_dir')
def run_snakemake(dry_run, use_sample_files, cores, genome_build, aligner, cluster, nodes, fastq_dir, reference_fasta, group1, group2, output_dir):

    cli.snakemake.run_snakemake(dry_run, use_sample_files, cores, genome_build, aligner, cluster, nodes, fastq_dir, reference_fasta, group1, group2, output_dir)
